...
def export_to_json(input_file, output_file):
    ...
    with open(input_file, 'r') as file:
        ...
        for row in reversed(reader[4:]):
            ...
            # PROCESS THE DATA HERE ¿ENCODING TYPE?
            ''''
            row_dict = dict(zip(headers[1], row))
            row_dict['SamiData'] = encode_base64(row_dict['SamiData'])
            row_dict['SamiMessage'] = encode_base64(row_dict['SamiMessage'])
            data.append(row_dict)  # Use the second line of headers for the data keys
            '''
...
if __name__ == "__main__":
    input_file = 'temporal_mds/BoiaBarcelona_Sami.dat'
    ...
    output_dir = 'Somorrostro/Sami'
    os.makedirs(output_dir, exist_ok=True)  # Create the directory if it doesn't exist
    output_file = os.path.join(output_dir, f'Sami_{year_month}.json')
